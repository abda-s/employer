...
function Education() {
  ...
  const { fetchData: editEducationData } = useAxios({
    url: `/cv/edit-education`,
    method: 'PUT',
    manual: true,
  });

  const { fetchData: addEducationData } = useAxios({
    url: `/cv/add-education`,
    method: 'PUT',
    manual: true,
  });

  const { fetchData: deleteEducationData } = useAxios({
    url: `/cv/education/${indexOfItem}`,
    method: 'DELETE',
    manual: true,
  });

  const submitEdit = async (institutionName, degree, graduationYear) => {
    const params = { educationIndex: indexOfItem, institutionName, degree, graduationYear };
    const result = await editEducationData({ body: params });
    if (!result.error) {
      setIsEditMode(false);
      setIndexOfItem(null);
  ...
  const submitAdd = async (institutionName, degree, graduationYear) => {
    const params = { institutionName, degree, graduationYear };
    const result = await addEducationData({ body: params });
    if (!result.error) {
      setIsEditMode(false);
  ...
  const deleteItem = async (index) => {
    const result = await deleteEducationData();
    if (!result.error) {
      setIsEditMode(false);
